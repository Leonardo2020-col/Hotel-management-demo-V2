...
// Actualizar solo la función handleDeleteGuest en Guests.jsx
...
const handleDeleteGuest = async (guestId) => {
  try {
    ...
    const confirmed = window.confirm(
      ...
      '• Si tiene historial de reservas completadas, se marcará como inactivo\n' +
    ...
    // Limpiar selección si el huésped fue procesado
    setSelectedGuests(prev => prev.filter(id => id !== guestId));
    
    // Mensaje de éxito basado en lo que realmente pasó
    if (result && result.status === 'inactive') {
      toast.success(`${guestName} desactivado exitosamente (tenía historial de reservas)`);
    } else {
    ...
    let errorMessage = 'Error desconocido al procesar el huésped';
    
    if (error.message) {
      if (error.message.includes('reserva(s) activa(s)')) {
        errorMessage = error.message;
      } else if (error.message.includes('registros relacionados')) {
        errorMessage = 'El huésped tiene registros relacionados y se ha marcado como inactivo en su lugar';
        // En este caso, recargar los datos para reflejar el cambio
        setTimeout(() => {
      ...
      } else if (error.message.includes('Error al desactivar')) {
        errorMessage = 'Error al desactivar el huésped: ' + error.message;
      } else {
        errorMessage = error.message;
      }
    }
    
    // Usar warning en lugar de error si fue marcado como inactivo
