// =====================================================
// 📄 PÁGINA COMPLETA DE RESERVACIONES
// =====================================================

// src/pages/Reservations.jsx - CÓDIGO COMPLETO ACTUALIZADO
...
const Reservations = () => {
  ...
  const canWrite = hasPermission('checkin') || hasPermission('reservations') || 
                   hasRole('recepcion') || hasRole('administrador');
  ...
  console.log('🔍 Permisos del usuario:', {
    ...
    isAdmin, 
    isReception,
    userRole: user?.userInfo?.role?.name,
    permissions: user?.userInfo?.role?.permissions
  });

  // Hook principal de reservas ACTUALIZADO con Supabase
  const {
    reservations,
    allReservations,
    loading,
  ...
  // Debug inicial
  useEffect(() => {
    console.log('🔍 Reservations component mounted with Supabase integration');
    console.log('👤 User permissions:', {
      canWrite,
      isAdmin,
      isReception,
      userEmail: user?.email,
      branchId: user?.userInfo?.user_branches?.[0]?.branch_id
    });
  }, [canWrite, isAdmin, isReception, user]);

  // Verificar errores del hook
  useEffect(() => {
    if (error) {
  ...
  const handleCreateReservation = async (reservationData) => {
    ...
    try {
      console.log('🏨 Creating reservation with Supabase:', reservationData);
      
      // Calcular monto total si no viene incluido
      const totalAmount = reservationData.totalAmount || 
                         (reservationData.nights * (reservationData.room?.base_rate || reservationData.room?.rate || 0));
      ...
      await createReservation(enhancedData);
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-medium ml-2">{reservation.room?.number} - {reservation.room?.type || 'Estándar'}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span className="font-medium ml-2 capitalize">{reservation.source || 'Direct'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-medium ml-2">{formatCurrency(reservation.rate || 0)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Total:</span>
                  <span className="font-medium ml-2">{formatCurrency(reservation.totalAmount || 0)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Pagado:</span>
                  <span className="font-medium ml-2">{formatCurrency(reservation.paidAmount || 0)}</span>
                </div>
                <div>
                  <span className="text-gray-600">Pendiente:</span>
                  <span className="font-medium ml-2 text-red-600">
                    {formatCurrency((reservation.totalAmount || 0) - (reservation.paidAmount || 0))}
                  </span>
                </div>
                <div>
                  <span className="text-gray-600">Estado de pago:</span>
                  <span className={`ml-2 px-2 py-1 rounded-full text-xs font-medium ${
                    (reservation.paidAmount || 0) >= (reservation.totalAmount || 0)
                      ? 'bg-green-100 text-green-800'
                      : (reservation.paidAmount || 0) > 0 
                      ? 'bg-yellow-100 text-yellow-800'
                      : 'bg-red-100 text-red-800'
                  }`}>
                    {(reservation.paidAmount || 0) >= (reservation.totalAmount || 0) ? 'Pagado' : 
                     (reservation.paidAmount || 0) > 0 ? 'Parcial' : 'Pendiente'}
                  </span>
                </div>
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {isAdmin && !isReception
              ...
              : canWrite 
              ? 'Administra todas las reservas del hotel con Supabase'
              : 'Visualiza las reservas del hotel'
            }
          </p>
          {user && (
            <p className="text-sm text-gray-500 mt-1">
              Conectado como: {user.email} • Sucursal: {user.userInfo?.user_branches?.[0]?.branch?.name || 'N/A'}
            </p>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-25 flex items-center justify-center z-40">
          <div className="bg-white rounded-lg p-6 shadow-xl">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-gray-700 font-medium">Procesando operación...</span>
            </div>
          </div>
        </div>
      )}
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="fixed bottom-4 right-4 bg-black text-white p-3 rounded-lg text-xs max-w-xs">
          ...
          <div>Total reservas: {reservations.length}</div>
          <div>Filtradas: {filteredReservations.length}</div>
          <div>Loading: {loading ? 'Sí' : 'No'}</div>
          <div>OperationLoading: {operationLoading ? 'Sí' : 'No'}</div>
          <div>Permisos: {canWrite ? 'Escritura' : 'Solo lectura'}</div>
          <div>Rol: {user?.userInfo?.role?.name || 'N/A'}</div>
          <div>Sucursal: {user?.userInfo?.user_branches?.[0]?.branch?.name || 'N/A'}</div>
