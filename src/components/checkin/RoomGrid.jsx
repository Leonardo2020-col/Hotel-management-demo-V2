// src/components/checkin/RoomGrid.jsx
import React from 'react';
import { Bed, ShoppingCart, ChevronRight } from 'lucide-react';
...
const RoomGrid = ({ 
  ...
  const getRoomStatusColor = (status) => {
    switch (status) {
      ...
      case 'occupied': return 'bg-red-500 hover:bg-red-600 text-white cursor-not-allowed';
    ...
    <>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {floorRooms[selectedFloor].map((room) => {
          const isClickable = checkoutMode ? room.status === 'occupied' : room.status !== 'occupied';
