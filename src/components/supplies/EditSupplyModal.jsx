...
const schema = yup.object().shape({
  ...
  currentStock: yup.number().min(0, 'El stock debe ser positivo').required('El stock actual es obligatorio'),
...
const EditSupplyModal = ({ isOpen, onClose, onSubmit, supply, categories, suppliers }) => {
  ...
  // Cargar datos del insumo al abrir el modal
  useEffect(() => {
    if (isOpen && supply) {
      reset({
        name: supply.name,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Editar Insumo</h2>
            <p className="text-gray-600 mt-1">Modificar información del insumo: {supply.name}</p>
          </div>
          <button
            onClick={handleClose}
          ...
          <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
            ...
            {supply && (
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <h4 className="font-semibold text-gray-900 mb-3">Cambios realizados</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {watchedCurrentStock !== supply.currentStock && (
                    <div>
                      <span className="text-gray-700">Stock:</span>
                      <span className="ml-2">
                        <span className="text-red-600">{supply.currentStock}</span>
                        <span className="mx-2">→</span>
                        <span className="text-green-600">{watchedCurrentStock}</span>
                      </span>
                    </div>
                  )}
                  {watchedUnitPrice !== supply.unitPrice && (
                    <div>
                      <span className="text-gray-700">Precio:</span>
                      <span className="ml-2">
                        <span className="text-red-600">S/ {supply.unitPrice}</span>
                        <span className="mx-2">→</span>
                        <span className="text-green-600">S/ {watchedUnitPrice}</span>
                      </span>
                    </div>
                  )}
                </div>
                ...
                {watchedUnitPrice > 0 && watchedCurrentStock >= 0 && (
                  <div className="mt-3 pt-3 border-t border-gray-300">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-gray-700">Valor anterior:</span>
                        <span className="font-semibold text-gray-900 ml-2">
                          S/ {(supply.currentStock * supply.unitPrice).toFixed(2)}
                        </span>
                      </div>
                      <div>
                        <span className="text-gray-700">Nuevo valor:</span>
